...
//#ifdef NODE_EXTEND
    ...
    Handle<Value> argv[] =
    {
        Number::New(msg.channel.header.stream_id),
        Boolean::New(true),
        Number::New(buf_flag)};
    ...
    case 7:
        {
            ...
            Handle<Value> argv[] =
            {
                String::New("ping_pong_request"),
                Number::New(0),
                Number::New(msg.protocol->total_recved_size)};
...
void throwup_av_info(RtmpClient *client, bls_message_t *msg)
{
    ...
    Handle<Value> argv[] =
    {
        String::New(av_type),           //av类型
        Boolean::New(is_key),         //是否是关键帧
        Boolean::New(is_sh),         //是否是avc sh
        Number::New(msg->channel.header.timestamp),           //时间戳
        Number::New(msg->channel.header.msg_len),             //数据大小
    };
