...
void on_new_connection(uv_stream_t *server, int status)
{
    ...
    if (status)
    {
        SYS_WARNING("get a new client error. status %d error: %s",
                status, uv_err_name(uv_last_error(uv_default_loop())));
        return;
    }
...
int RtmpServer::start()
{
    ...
    uv_tcp_bind(&uv_server, bind_addr);
    listen_result
    ...
    if (listen_result)
    {
        SYS_FATAL("can not listen port %d. err: %s",
                port, uv_err_name(uv_last_error(main_loop)));
        return -1;
    }
...
void RtmpServer::accept_client()
{
    ...
    else
    {
        SYS_WARNING("accept new client fail. error: %s",
                uv_err_name(uv_last_error(main_loop)));
        bls_close_tcp(client);
