...
#define COMLOG_DEVICE_CRONOCUT      "DATA_CRONOCUT"			//cut log according to the natural time
...
struct com_device_t
{
    ...
    int cronocut;       // whether to cut the log file according to the natural time, defaultly 0
 ...
 *
 * @param [in] events   : int ÈÕÖ¾µÈ¼¶id
 * @return  ³É¹¦·µ»ØCategory¶ÔÏóÖ¸Õë£¬ÆäËû·µ»Ønull
 * @retval
 * @see
 * @author zhaoxin08
 * @date 2014/07/30 10:55:46
**/
#include "category.h"
comspace::Category* com_writelog_stream(int events);

/**
 * ÏÂÃæ¼¸¸öºêµÄÓÃ·¨ÈçÏÂ£º
 * CFATAL_LOG_S << "hello" << " world!" << endl;
 * or
 * CFATAL_LOG_S << "hello" << " world!";
 * CFATAL_LOG_S << endl;
 *
 * ²»ÍÆ¼öÏÂÃæµÄÓÃ·¨£º
 * CFATAL_LOG_S << "hello";
 * CDEBUG_LOG_S << " world!";	// ÉÏÃæµÄSFATAL_LOGµÄlogÄÚÈİ»¹Ã»ÓĞÍ¨¹ıÏÂÃæÒ»ĞĞµÄ´úÂë±»flush³öÈ¥Ç°£¬²»ÒªÇĞ»»log level
 * CFATAL_LOG_S << endl;
 * Èç¹ûÊ¹ÓÃÁËÉÏÃæµÄÓÃ·¨£¬ÔòlogÎÄ¼şÖĞ²»»áÓĞµ¥¶ÀµÄÒ»ĞĞ
 * µÄdebug¼¶±ğµÄ"world"ÈÕÖ¾£¬Ëü»á±»µ±×öfatal¼¶±ğµÄÈÕÖ¾±»¸½¼ÓÔÚ"hello"ºóÃæ
 **/
#define CFATAL_LOG_S     *(com_writelog_stream(COMLOG_FATAL))
#define CWARNING_LOG_S   *(com_writelog_stream(COMLOG_WARNING))
#define CNOTICE_LOG_S    *(com_writelog_stream(COMLOG_NOTICE))
#define CTRACE_LOG_S     *(com_writelog_stream(COMLOG_TRACE))
#define CDEBUG_LOG_S     *(com_writelog_stream(COMLOG_DEBUG))
